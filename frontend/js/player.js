...
function openPlayer(list, index, playlistVideo = null) {
    ...
    currentIndex = Math.max(
        0,
        Math.min(index || 0, currentList.length - 1)
    );
...
function setupPlaylistVideo() {
    ...
    /*
        Only change the video source when
        the actual URL changes.

        This prevents the video from constantly
        restarting or flashing.
    */
...
function startPlaylistVideo() {
    ...
    /*
        Only call play() when the video is
        actually paused.
    */
    if (playerVideo.paused) {
        ...
        if (
            videoPromise &&
            typeof videoPromise.catch === "function"
        ) {

            videoPromise.catch(error => {

                console.warn(
                    "Playlist video playback failed:",
                    error
                );
...
function previousSong() {
    ...
    /*
        ...
        restart the current song instead of
...
function startHideTimer() {
    ...
    /*
        Only playlist players get auto-hide.

        Normal songs ALWAYS keep their
        controls visible.
    */
    ...
    hideUITimer = setTimeout(() => {
        ...
        if (
            ...
            /*
                Keep the playlist background video
                playing while the UI is hidden.
            */
            keepPlaylistVideoPlaying();
        }
...
/*
    Keep playlist video playing while the
    controls are hidden.
*/
function keepPlaylistVideoPlaying() {
    ...
    if (playerVideo.paused) {
        ...
        if (
            ...
            videoPromise.catch(error => {
                ...
                console.warn(
                    "Could not resume playlist video:",
                    error
                );
...
function showPlayerUI() {
    ...
    /*
        Restart the 6-second timer for
        playlist mode.
    */
    startHideTimer();
}
        ...
        async (event) => {
            ...
            try {
                ...
                if (
                    ...
                    /* Use server's returned like count. */
                    if (
                        result &&
                        typeof result.likes !==
                        "undefined"
...
if (player) {
    ...
    [
        ...
        /*
            Listen on the document instead of only
            the player. This allows the controls to
            return when the hidden player is moved
            or interacted with.
        */
